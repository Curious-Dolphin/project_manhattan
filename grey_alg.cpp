...
// save ROI
int m_x_roi = g_roi_IR.x;
int m_y_roi = g_roi_IR.y;
int m_w_roi = g_roi_IR.width;
int m_h_roi = g_roi_IR.height;
...
uint idx_pos_sup = 0;
uint idx_pos_inf = 0;
double th_canny_grow;
double th_canny_enuc;
std::string file_name;
// --- LINE SELECTION ---
float x_edge_lineR = m_w_roi/2;
cv::Point_<int> p_edgeR(cvRound(x_edge_lineR),0);
//p_edgeR.x = cv::cvRound(x_edge_lineR);                qua faccio la cafonata con il punto
...
cv::Point Grey_Alg::analyse_image(const cv::Mat &img, uint64_t t_img_us_syn)
{
    ...
    // Returns  : position of seam in the full image
        ...
        try
        {
            std::cout << "ALG --- Image n. " << m_count_frame << " at " << t_img_us_syn << " rob_vec at: ";
        ...
        try{
            ...
            cv::putText(img_dist, "Enuc: " + std::to_string(cvRound(th_canny_enuc)) + "(" + std::to_string(cvRound(m_corr_cann)) + ")", cv::Point(img_dist.cols*1/3, img_dist.rows*1/2),
                        cv::FONT_HERSHEY_DUPLEX, 1, CV_RGB(118, 185, 0), 2);
            file_name = "/srv/ftp/seam-tracker/img/dist/img_" + std::to_string(m_count_frame) + "-ts_" + std::to_string(t_img_us_syn) + ".bmp";
            ...
            file_name = "/srv/ftp/seam-tracker/img/cann/img_" + std::to_string(m_count_frame) + "-ts_" + std::to_string(t_img_us_syn) + ".bmp";
        ...
        else
        {
            ...
            for (; i_gap_x != m_y_gapW_b.end(); i_gap_x++, i_gap_y++, i_gap_good++)
            {
                if(m_debug_mode){
...
//            }
        ...
        if(m_debug_mode)
        {
            ...
            // Save
            file_name = "/srv/ftp/seam-tracker/img/seam/img_" + std::to_string(m_count_frame) + "-ts_" + std::to_string(t_img_us_syn) + ".bmp";
            cv::imwrite(file_name, img_seam);
        }
        ...
        double w_gapW = m_w_gapI_b.back()*g_px_res/1000.0*cos( (m_max_theta + m_min_theta)/2 - CV_PI/2);
        {
            ...
            m_estimate.p_seamW = p_gapW;        // mod V
...
{
    std::cout << "ALG --- LinePlot2 start " << std::endl;
    cv::LineIterator line_iter(m_img_blur, p1, p2);
    ...
    std::array<cv::Point,2> p_widthI {p_bigger, p_lesser};
    std::cout << "ALG --- LinePlot2 ends" << std::endl;
    return p_widthI;
}
